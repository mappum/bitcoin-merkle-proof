...
test('invalid merkleblocks', function (t) {
  t.test('not all hashes used', function (t) {
    try {
      merkleTree({
        flags: [ 0xff ],
        hashes: [
      ...
      t.pass('error thrown')
      t.equal(e.message, 'Tree did not consume all hashes')
    }
    t.end()
  })

  t.test('invalid merkle root in header', function (t) {
    try {
      merkleTree({
        flags: [ 109, 117, 0 ],
        hashes: [
